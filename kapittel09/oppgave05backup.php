...
<body>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
        ...
        <input type="number" name="utgivelsesår" PLACEHOLDER="utgivelsesår">
    ...
    <?php
        if(isset($_REQUEST['lagtTil'])) {
            ...
            $sql = "INSERT INTO bøker (tittel, fk_forfID, utgivelsesår)
                    VALUES (:tittel, :f_id, :utgivelsesår)";
            $sp = $pdo->prepare($sql);

            $sp->bindParam(':tittel', $tittel, PDO::PARAM_STR);
            $sp->bindParam(':f_id', $f_id, PDO::PARAM_INT);
            $sp->bindParam(':utgivelsesår', $utgivelsesår, PDO::PARAM_INT);

            $tittel = $_REQUEST['tittel'];
            $f_id = $_REQUEST['ID'];
            $utgivelsesår = $_REQUEST['utgivelsesår'];
